...
say() {
  ...
  case "$LANG_CHOICE:$key" in
    ...
    ru:apk_locked) echo "База apk занята другим процессом. Завершите старый установщик/ apk и запустите снова." ;;
    en:apk_locked) echo "The apk database is locked by another process. Stop the old installer/apk process and run again." ;;
...
require_file() {
  f="$1"
  if [ ! -f "$f" ]; then
    echo "ERROR: $(say missing_file): $f" >&2
    exit 1
  fi
}
...
wait_apk_free() {
  # Не удаляем lock-файлы вслепую: если apk реально работает, можно повредить базу.
  # Ждём до 30 секунд, затем показываем процессы.
  i=0
  while [ "$i" -lt 30 ]; do
  ...
  echo "Fix:" >&2
  echo "  jobs -l" >&2
  echo "  kill %1        # если старый установщик остановлен через Ctrl+Z" >&2
  echo "  ps w | grep apk" >&2
  echo "  kill <PID>     # если завис apk/update" >&2
  exit 1
}

install_packages() {
  command -v apk >/dev/null 2>&1 || {
    echo "ERROR: $(say need_apk)" >&2
    exit 1
  }
  ...
  # ip-full/bridge-utils могут отсутствовать в отдельных сборках, поэтому ставим их best-effort.
...
service_running() {
  [ -x "$INIT_FILE" ] && "$INIT_FILE" status 2>/dev/null | grep -qi running
}
...
install_routerdash() {
  step 1 6 "$(say step_pkg)"
  install_packages

  step 2 6 "$(say step_copy)"
  ...
  step 3 6 "$(say step_cfg)"
  write_default_config

  step 4 6 "$(say step_nlbw)"
  configure_nlbwmon

  step 5 6 "$(say step_start)"
  if ! start_routerdash_service; then
    echo "WARNING: $(say service_fail)" >&2
  fi
