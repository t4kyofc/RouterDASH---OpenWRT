...
1. Лаунчер предлагает выбрать язык установки
2. Скачивает `install.sh`, `routerdash.py` и `routerdash.init`
3. Запускает локальный установщик
4. Включает и запускает сервис `routerdash`

Теперь установщик совместим с BusyBox-окружением OpenWrt и не требует внешней утилиты `install`.
